...
%% Parameters
K_max = 6;  % Number of subbands
SNR_list = [-20:2:20];
acc_mat = zeros(K_max, length(SNR_list));
node_pot_list = zeros(2*K_max, 2);

for i = 1:2*K_max
    node_pot_list(i, 1) = rand;
    node_pot_list(i, 2) = 1 - node_pot_list(i, 1);
end
...
for K = 1:K_max
    ...
    Sigma = 2;  % Noise power
    acc_vec = zeros(1, length(SNR_list));
    
    for R = 1:length(SNR_list)
        SNR = SNR_list(R);
